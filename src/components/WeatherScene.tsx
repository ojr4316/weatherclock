...
export function WeatherScene({
  ...
  const timeLabel = settings.timeFormat === '12h' && settings.hideAmPm
    ? timeFormatter
        .formatToParts(now)
  ...
  const formattedTemperature = formatTemperature(temperature, settings.temperatureUnit)
    ...
    <main className="weather-app">
        ...
        <form className="settings-panel" aria-label="Display settings">
          ...
          <label>
            <input
              ...
              onChange={(event) => updateSettings({ showMoreWeatherInfo: event.target.checked })}
            />
          ...
          <label className={settings.timeFormat === '24h' ? 'is-disabled' : undefined}>
            <input
              type="checkbox"
              checked={settings.hideAmPm}
              disabled={settings.timeFormat === '24h'}
              onChange={(event) => updateSettings({ hideAmPm: event.target.checked })}
            />
            Hide AM/PM
          </label>
      ...
      >
        ...
        <div className="status-card">
          <span className={`weather-dot weather-dot--${weatherState}`} aria-hidden="true" />
          <span>{status}</span>
        </div>
